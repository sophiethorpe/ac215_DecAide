...
async def startup():
    ...
    global model, label_encoder, captioning_processor, captioning_model
    model = load_model()
    ...
    local_model_dir = os.path.join(current_dir, "blip_model")

    # Initialize the image captioning model (BLIP in this case)
    captioning_processor = BlipProcessor.from_pretrained(local_model_dir)
    captioning_model = BlipForConditionalGeneration.from_pretrained(local_model_dir)
...
async def generate_caption(file: UploadFile = File(...)):
    ...
    try:
        ...
        output = captioning_model.generate(
            **inputs,
            max_new_tokens=100,
            num_beams=5,  # Beam search for coherent captions
